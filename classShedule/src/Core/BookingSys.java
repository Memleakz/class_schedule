...
public class BookingSys {
    ...
    {
	//Load DATA!
	this.Rooms = new ArrayList<LocationInterface>();
	this.Courses = new ArrayList<CourseInterface>();
	this.specialClosePeriod = new ArrayList<Period>();
    }
    ...
    public List<CourseInterface> AssignRoomsForCourses() {
	...
	Period BookingPeriod = new Period(this.SemesterStart,this.SemesterEnd);
	for (CourseInterface c : this.Courses) {
		...
		List<BookingLocationsInterface> abooks = AttemBookCourseTimes(c, r, "bedst",Total_needed_bookings,BookingPeriod);
		...
		for (LocationInterface r : allPossibleRooms) {
		    known_bookings = l.get(r);
		    List<BookingLocationsInterface> abooks = AttemBookCourseTimes(c, r, "medium",Total_needed_bookings,BookingPeriod);
		    if (known_bookings == null) {
			l.put(r, abooks);
		    } else {
			known_bookings.addAll(abooks);
		...
		for (LocationInterface r : allPossibleRooms) {
		    known_bookings = l.get(r);
		    List<BookingLocationsInterface> abooks = AttemBookCourseTimes(c, r, "emergency",Total_needed_bookings,BookingPeriod);
		    if (known_bookings == null) {
			l.put(r, abooks);
		    } else {
			known_bookings.addAll(abooks);
    ...
    public Room findBedstMatchRoom(Course c, List<Room> rooms) {
    ...
    public  List<BookingLocationsInterface> AttemBookCourseTimes(CourseInterface c, LocationInterface r, String target_periode,int Total_needed_bookings,Period BookingPeriod) {
	...
	while (Total_needed_bookings != 0) {
		...
		Lecturer adam = new Lecturer("00001", "Adam Adamsen", new ArrayList<Period>(), new ArrayList<CourseInterface>());
    ...
    public int cancelBooking(BookingLocationsInterface booking,boolean attemptRebook)
    {
	for(LocationInterface r : this.Rooms)
	{
	...
	if(attemptRebook)
	{
	    ...
	    Period booking_period = new Period(booking.getPeriodOfBooking().getStartDate().plusDays(1).format(formatter),this.SemesterEnd);
	    for(LocationInterface r : allPossibleRooms)
	    {
		//we just need to book 1..
		List<BookingLocationsInterface> bookings = AttemBookCourseTimes(booking.getCourse(),r,"bedst",1,booking_period);
		if(bookings.size() == 1)
		{
